...
def get_weights(net, bit_pattern, character, inputs_per_type, fire_at):
    ...
    If the net has 2 layers, the, the weights are set from the inputs
    to the outputs.

    If it has 3, the weights are set from the inputs to the hidden
    layer, and the second interlayer will pretty much just feed
    straight through, with slight randomisation. Clever behaviour has
    to be evolved later.
    """
    #XXX not pretty
